...
class SSDLoss():
    def __init__(self):
        ...
        self.cls_base = tf.losses.CategoricalCrossentropy(reduction=tf.losses.Reduction.NONE)
    ...
    def cls_loss(self, gt_labels: tf.Tensor, pred_labels: tf.Tensor) -> tf.Tensor:
        ...
        sorted = tf.argsort(tf.argsort(masked, direction="DESCENDING"))
        neg_mask = tf.cast(tf.less(sorted, tf.expand_dims(neg_pos_boxes, axis=1)), dtype=tf.float32)
